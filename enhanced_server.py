...
async def js_test_page(request: Request):
    ...
    return templates.TemplateResponse("js-test.html", {
        "request": request,
        "title": "JavaScript测试",
        "version": "1.0.0"
    })
...
async def js_class_test_page(request: Request):
    ...
    return templates.TemplateResponse("js-class-test.html", {
        "request": request,
        "title": "JavaScript类测试",
        "version": "1.0.0"
    })
...
async def simple_js_test_page(request: Request):
    ...
    return templates.TemplateResponse("simple-js-test.html", {
        "request": request,
        "title": "简单JavaScript测试",
        "version": "1.0.0"
    })
...
async def enterprise_frontend(request: Request):
    ...
    return templates.TemplateResponse("enterprise.html", {
        "request": request,
    ...
    try:
        ...
        async for db in get_db():
            ...
            result = {
                ...
                "model": settings.MODEL_NAME,
                "timestamp": datetime.now().isoformat(),
                "tokens_used": response.usage.total_tokens if response.usage else len(user_content.split()),
...
async def test_tools(request: ToolTestRequest, http_request: Request):
    ...
    try:
        ...
        try:
            ...
            success = False
            error_message = str(e)
            api_logger.error("工具测试执行失败: %s, 错误: %s", request.tool_name, str(e))
