...
class MemorySessionService:
    ...
    def __init__(self):
        # 会话存储: {session_id: {"system_prompt": str, "messages": List[Dict]}}
    ...
    def get_or_create_session_prompt(self, session_id: str, requested_prompt: Optional[str] = None) -> str:
        """获取或创建会话的系统提示词"""
        if requested_prompt and self.sessions[session_id]["system_prompt"] != requested_prompt:
            self.sessions[session_id]["system_prompt"] = requested_prompt
            logging.info(f"为会话 '{session_id}' 更新系统提示词")
    ...
    def get_history(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        # 智能截断逻辑
        truncated_messages = messages[-self.max_history_messages:]
        
        # 检查是否有不完整的工具调用链
        while len(truncated_messages) > 0:
            oldest_message = truncated_messages[0]
            
            # 如果最老的消息是工具调用链的一部分，需要扩展窗口
            if (oldest_message.get("role") == "tool" or 
                (oldest_message.get("role") == "assistant" and "tool_calls" in oldest_message)):
                
                # 找到这个工具调用链的开始
                chain_start_index = messages.index(oldest_message)
                if chain_start_index > 0:
                    # 向前扩展一条消息
                    truncated_messages.insert(0, messages[chain_start_index - 1])
                else:
                    break
            else:
                break
        
        logging.info(f"为会话 '{session_id}' 获取 {len(truncated_messages)} 条历史消息")
    ...
    def update_history(self, session_id: str, new_messages: List[Dict[str, Any]]):
        ...
        # 如果超过最大限制，进行清理
        if len(self.sessions[session_id]["messages"]) > self.max_history_messages * 2:
            # 保留最近的消息
            self.sessions[session_id]["messages"] = self.sessions[session_id]["messages"][-self.max_history_messages:]
        
        logging.info(f"为会话 '{session_id}' 添加了 {len(new_messages)} 条新消息")
...
# 使用真实的工具注册表 (已通过导入 heimdall.py_ai_core.tools 自动注册)
...
async def test_llm_capability(request: LLMTestRequest):
    ...
    **功能:**
    ...
    try:
        ...
        # 获取系统提示词
        system_prompt = session_service.get_or_create_session_prompt(session_id, request.system_prompt)
        
        # 构建消息列表，包含历史记录
        history_messages = session_service.get_history(session_id)
        ...
        session_service.update_history(session_id, [current_user_message, assistant_message])
        
        # Debug logging
        logger.info(f"DEBUG: session_id value: {session_id}")
        logger.info(f"DEBUG: session_id type: {type(session_id)}")
...
async def test_llm_with_tools(request: LLMWithToolsRequest):
    """
    ...
    **功能:**
    ...
    try:
        ...
        session_service.update_history(session_id, messages_to_save)
